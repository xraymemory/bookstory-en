...
Begin VB.MDIForm MDIForm1
  ...
  Begin Menu MMenu
    Caption = "ÉÅÉjÉÖÅ[(&M)"
    Begin Menu MNewGame
      Caption = "êVÇµÇ≠ÇÕÇ∂ÇﬂÇÈ(&N)"
    End
    Begin Menu MMinasu03
      Caption = "-"
    End
    Begin Menu MSave
      Caption = "ÉQÅ[ÉÄÇÃï€ë∂(&S)"
    End
    Begin Menu MLoad
      Caption = "ÉQÅ[ÉÄÇÃì«çû(&L)"
    End
    Begin Menu MMinasu02
      Caption = "-"
    End
    Begin Menu MEnd
      Caption = "ÉQÅ[ÉÄèIóπ(&E)"
    End
  End
  Begin Menu MAction
    Caption = "çsìÆ(&A)"
    ...
    Begin Menu MTenin
      Caption = "ìXàıä÷åW(&M)"
      Begin Menu MTeen
        Caption = "ìXàıÇåŸóp(&T)"
      End
      Begin Menu MKaiko
        Caption = "ìXàıÇâåŸ(&O)"
      End
      Begin Menu MTeninSetup
        Caption = "ìXàıÇÃê›íË(&S)"
      End
    End
    Begin Menu MEigyouHousin
      Caption = "Ç®ìXÇÃâcã∆ï˚êj(&E)"
      Begin Menu MEigyou
        Caption = "Ç®ìXÇÃÉTÅ[ÉrÉXì‡óe(&E)"
      End
      Begin Menu MTatiyomi
        Caption = "óßÇøì«Ç›ëŒçÙ(&T)"
      End
      Begin Menu MHoken
        Caption = "ï€åØÇ…ì¸ÇÈ(&H)"
      End
    End
    Begin Menu MSouzi
      Caption = "Ç®ìXÇë|èúÇ∑ÇÈ(&S)"
    End
    Begin Menu MSenden
      Caption = "Ç®ìXÇÃêÈì`ÇÇ∑ÇÈ(&S)"
    End
    Begin Menu MTakarakuzi
      Visible = 0   'False
      Caption = "ïÛÇ≠Ç∂ÇîÉÇ§(&T)"
    End
    Begin Menu MGinkou
      Caption = "ã‚çsÇ÷çsÇ≠(&G)"
    End
    Begin Menu MTaisin
      Caption = "ëœêkçHéñÇÇ∑ÇÈ(&T)"
    End
    Begin Menu MSyoubou
      Caption = "è¡ñhé‘ÇåƒÇ‘(&S)"
    End
    Begin Menu MKibo
      Caption = "Ç®ìXÇÃãKñÕägëÂ(&K)"
    End
  End
  Begin Menu MInformation
    ...
    Begin Menu MGurahu
      ...
      Begin Menu MRiekiGurahu
        Caption = "Ç®ìXÇÃóòâvÉOÉâÉt(&R)"
      End
      Begin Menu MBookGurahu
        Caption = "ñ{ÇÃîÑÇËêîÉOÉâÉt(&G)"
      End
    End
    Begin Menu MGuest
      Caption = "Ç®ãqÇ≥ÇÒÇÃà”å©(&G)"
    End
    Begin Menu MGRead
      Caption = "ã∆äEêVï∑Çì«Çﬁ(&R)"
    End
    Begin Menu MNyuuka
      Caption = "ì¸â◊ÉXÉPÉWÉÖÅ[Éã(&N)"
    End
    Begin Menu MTinfor
      Caption = "ìXàıèÓïÒ(&T)"
    End
    Begin Menu MRieki
      Caption = "Ç®ìXÇÃóòâv(&S)"
    End
    Begin Menu MBookInfo
      Caption = "Ç®ìXÇÃèÓïÒ(&B)"
    End
    Begin Menu MBookSyousai
      Caption = "Ç®ìXÇÃè⁄ç◊èÓïÒ(&S)"
    End
  End
  Begin Menu MWindow
    Caption = "ÉEÉCÉìÉhÉE(&W)"
    Begin Menu MVisual
      Caption = "ÉrÉWÉÖÉAÉãÉEÉCÉìÉhÉE(&V)"
    End
    Begin Menu MSwitch
      Caption = "ÉEÉCÉìÉhÉEêÿÇËë÷Ç¶(&W)"
      WindowList = -1  'True
    End
  End
  Begin Menu MSetup
